...
Make sure that you understand what the last line did. We made some subjects respond faster tout court, with a stronger increase in RTs for the object relative clause condition.
...
d. Compare the fitted models using leave-one-out cross-validation. Use `loo` package by calling `loo::loo(model_1, model_2)`. This function returns the loo-values as discussed in class multiplied by $-2$ (thus making the loo score play in the same ball park as standard information criteria). Interpret the outcome of this model comparison. Which model is better? Check the estimated standard error of the loo-IC scores and validate whether the result of the model comparison is trustworthy.
