...
\begin{tikzpicture}[node distance = 2cm, 
    ...
    \node[circle, draw = black, above left of = theta] (omega) {$\omega$};

    \node[circle, draw = black, above right of = theta] (kappa) {$\kappa$};
    ...
    \begin{scope}[xshift = 5.75cm, node distance = 1.33cm]
      ...
      \node[above of = nProb] (thetaProb) {$\theta_i \sim \text{Beta}(\omega((\kappa - 2) + 1),
        (1-\omega)(\kappa-2)+1)$}; 

      \node[above of = thetaProb] (kappaProb) {$\kappa - 2 \sim \text{Gamma}(0.01,0.01)$};

      \node[above of = kappaProb] (omegaProb) {$\omega \sim \text{Beta}(1,1)$};
