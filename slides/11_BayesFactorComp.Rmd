...
\text{BF}(M_0 > M_1) & = \frac{P(D \mid M_0)}{P(D \mid M_1)} \\
  ...
  & = \frac{{{N}\choose{k}} 0.5^{N}}{{{N}\choose{k}} \int_0^1  \theta^{k} \, (1-\theta)^{N - k} \text{ d}\theta} \\
...
\text{BF}(M_0 > M_1) & = \frac{P(D \mid M_0)}{P(D \mid M_1)} \\
  ...
  & = {{N}\choose{k}} 0.5^{N} (N + 1) \approx `r round(0.5^7 * 0.5^(24-7) / beta(8, 25-7),3)`
\end{align*}
$$
...
$$
\begin{align*}
\frac{1}{D} & = \frac{P(\theta \mid D)}{P(D \mid \theta) P(\theta)}\\
